...
int tps65132b_read_reg(struct i2c_client *client, u8 *buf, u8 *data)
{
	...
	pr_info("[MSM_LCD] tps65132b_read_reg retries=%d,ret=%d\n", retries, ret);
	if (ret != 2) {
...
int tps65132b_write_reg(struct i2c_client *client, u8 *buf, int len)
{
	...
	pr_info("[MSM_LCD] tps65132b_write_reg tries=%d,ret=%d\n", tries, err);
	if (err != 1) {
...
void tps65132b_set_vsp_vsn_level(int level)
{
	...
	/*pr_info("MSM_LCD tps65132b_set_vsp_vsn_level probe=%d\n", ti65132b_probe);*/
	if (ti65132b_probe) {
				...
				const struct i2c_device_id *id)
{
	...
	ti65132b_probe = true;
	pr_info("[MSM_LCD]tps65132b ti65132b_probe ok\n");
	return 0;
}
