...
#include <vendor/soc/qcom/board_id.h> /*@zte board id*/

#define IDLE 0
#define ACTIVE 1
#define USER_BUF_SIZE 100

#define SX9325_TAG					"==ZTE-SX9325=="
#define SX9325_DBG(fmt, args...)	printk(SX9325_TAG fmt, ##args)
#define SX9325_DBG2(fmt, args...)
#define SAR_GPIO_EINT_PIN	63
#define MSM_FONY_BOARD_ID   11
#define MSM_GARLIC_BOARD_ID   12
#define MSM_ILLENI_BOARD_ID   9
...
static struct smtc_reg_data sx9325_i2c_reg_setup[] = {
	...
	},
	{
		...
		.val = 0x00, /*disable ph1*/
	},
	{
		.reg = SX9325_AFE_PH2_REG,
#ifdef BOARD_IS_ILLENI
		.val = 0x00,
#else
		.val = 0x00,
#endif
...
#ifdef BOARD_IS_ILLENI
		.val = 0x21,/*enable PH0 and PH2 0x25*/
#else
		.val = 0x21,/*enable PH0*/
#endif
	},
...
struct sx93XX {
	...
	struct pinctrl_state		*sx9325_active;
	struct pinctrl_state		*sx9325_sleep;
	struct regulator			 *sx9325_vdd;
};

psx93XX_t g_sx9325_chip = 0;
unsigned char g_enable_log = 0;
int disable_sar_in_call = 0;
int need_input_index = -1;
static void sx9325_touchProcess(psx93XX_t chip);
...
static ssize_t status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pr_err("=status_show==read_register SX9325_STAT0_REG: Value: 0x%x\n", touchStatus);
					 ...
					 struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(chip->pdev, "Reading SX9325_IRQ_ENABLE_REG\n");
...
static void hw_init(psx93XX_t chip)
{
	...
	int num = ARRAY_SIZE(sx9325_i2c_reg_setup);

	/* configure device */
	dev_dbg(chip->pdev, "Going to Setup I2C Registers! chip->board_id (%d)\n", chip->board_id);
	if (chip) {
		while (i < num) {
			write_register(chip, sx9325_i2c_reg_setup[i].reg, sx9325_i2c_reg_setup[i].val);
			read_register(chip, sx9325_i2c_reg_setup[i].reg, &tempval);
			i++;
		}
	} else {
...
static int initialize(psx93XX_t chip)
{
	...
	/* perform a reset */
	/*write_register(chip,SX9325_SOFTRESET_REG,SX9325_SOFTRESET);*/
...
int sx93XX_init(psx93XX_t chip)
{
	...
	if (chip) {
		if (chip->sx9325_vdd)
		err = regulator_enable(chip->sx9325_vdd);

		if (chip->pinctrl) {
			err = pinctrl_select_state(chip->pinctrl, chip->sx9325_sleep);
			if (err) {
			...
			err = pinctrl_select_state(chip->pinctrl, chip->sx9325_active);
			if (err) {
...
#if 0
static int sx9325_fb_callback(struct notifier_block *nfb,
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_UNBLANK)	{
			if (g_sx9325_chip)
				manual_offset_calibration(g_sx9325_chip);
		}
	}
...
static struct notifier_block __refdata sx9325_fb_notifier = {
	.notifier_call = sx9325_fb_callback,
};
#endif
...
/*@zte board id, end*/
#endif
static int sx9325_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	board_id = sx9325_load_board_id();

	pr_info("%s, board_id %d E\n", __func__, board_id);
#ifdef BOARD_IS_ILLENI
	if (board_id <= MSM_ILLENI_BOARD_ID) {
		pr_info("%s, illeni board !\n", __func__);
	}
#else
#ifdef BOARD_IS_JULLY
	pr_info("%s, jully board !\n", __func__);
#else
#ifdef BOARD_IN_KANAS
	pr_info("%s, kanas board !\n", __func__);
#else
	pr_err("%s, board not support return E\n", __func__);
	return -ENODEV;
#endif
#endif
#endif
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_READ_WORD_DATA)) {
		SX9325_DBG("%s i2c not support read word data\n", __func__);
		return -EIO;
	}
	...
	if (IS_ERR_OR_NULL(chip->pinctrl)) {
		...
		chip->sx9325_active = pinctrl_lookup_state(chip->pinctrl, "sx9325_active");
		if (IS_ERR_OR_NULL(chip->sx9325_active))	{
		pr_err("%s, error pinctrl_lookup_state() for SX9325_ACTIVE\n", __func__);
		goto probe_error;
		}
	chip->sx9325_sleep = pinctrl_lookup_state(chip->pinctrl,	"sx9325_sleep");
	if (IS_ERR_OR_NULL(chip->sx9325_sleep))	{
		pr_err("%s, error pinctrl_lookup_state() for SX9325_SLEEP\n", __func__);
		goto probe_error;
		}
	}
...
static int sx9325_suspend(struct device *dev)
{
	...
	if (chip) {
	/*SX9325_DBG("%s report far when diabling sar\n", __func__);
...
static const struct i2c_device_id sx9325_input_id[] = {
	{"sx9325-input", 0},
	{},
};
MODULE_DEVICE_TABLE(i2c, sx9325_driver_id);

static struct i2c_driver sx9325_input_driver = {
	.driver		= {
		.name		= "sx9325-input",
		.owner		= THIS_MODULE,
	...
	.id_table	= sx9325_input_id,
};


module_i2c_driver(sx9325_input_driver);

MODULE_DESCRIPTION("sx9325 input");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("i2c:sx9325-input");
