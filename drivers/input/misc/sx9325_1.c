...
static ssize_t status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/*dump_register(chip);*/
#ifdef BOARD_IS_GARLIC
	if ((touchStatus & 0x04) && (touchStatus & 0x01)) {
		return snprintf(buf, 64, "6\n");/*main add wifi ant all near*/
...
#ifdef BOARD_IS_GARLIC
	pr_info("%s, garlic board !\n", __func__);

	if (board_id < MSM_GARLIC_BOARD_ID) {
