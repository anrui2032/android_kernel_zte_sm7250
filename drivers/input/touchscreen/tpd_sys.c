...
static ssize_t tp_sensibility_read(struct file *file,
					 char __user *buffer, size_t count, loff_t *offset)
{
	ssize_t len = 0;
	uint8_t data_buf[10] = {0};
...
static ssize_t tp_sensibility_write(struct file *file,
				const char __user *buffer, size_t len, loff_t *off)
{
	int ret = 0;
	unsigned int input = 0;
...
static const struct file_operations proc_ops_sensibility = {
	.owner = THIS_MODULE,
	.read = tp_sensibility_read,
	.write = tp_sensibility_write,
};
...
static void create_tpd_proc_entry(void)
{
	...
	tpd_proc_entry = proc_create(PROC_TOUCH_SENSIBILITY, 0664, tpd_proc_dir, &proc_ops_sensibility);
	if (tpd_proc_entry == NULL)
		pr_err("proc_create sensibility failed!\n");
...
static int __init tpd_class_init(void)
{
	...
	tpd_fw_cdev.get_sensibility = NULL;
	tpd_fw_cdev.set_sensibility = NULL;
	...
	tpd_fw_cdev.get_singleaod = NULL;
	tpd_fw_cdev.set_singleaod = NULL;
	...
	tpd_fw_cdev.sensibility_enable = 0;
	...
	tpd_fw_cdev.b_single_aod_enable = 0;
