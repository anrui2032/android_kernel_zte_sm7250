...
#define TP_POWER_OFF	"TP_POWER_STATUS=1"
#define TP_POWER_ON		"TP_POWER_STATUS=2"
#define TP_POWER_AOD	"TP_POWER_STATUS=3"
#define TP_POWER_TEMP	"TP_POWER_STATUS=4"
...
enum dynamic_config_id {
	...
	DC_ENABLE_GLOVE,
	DC_ENABLE_CLOSED_COVER = 0xc3,
	DC_ENABLE_FINGERPRINT_GESTURE = 0xdc,
};
...
struct syna_tcm_hcd {
	...
	spinlock_t fp_logo_lock;
	spinlock_t fp_sintap_lock;
	spinlock_t edge_pre_lock;
	...
	bool in_wakeup_gesture;
	bool gesture_set_change_in_sus;
#endif
	bool is_irq_wake;
/*add by zte for lp*/
	bool in_low_power;
/*add by zte for lp*/
	...
	unsigned int *baseline_data_words;
	unsigned int is_single_tap;
	unsigned int single_set_change_in_sus;
	...
	struct workqueue_struct *resume_suspend_workqueue;
...
static inline void tp_irq_wake(struct syna_tcm_hcd *tcm_hcd, bool enable)
{
	if (enable && !tcm_hcd->is_irq_wake) {
		enable_irq_wake(tcm_hcd->irq);
		tcm_hcd->is_irq_wake = true;
		pr_notice("syna: enable_irq_wake");
	} else if (!enable && tcm_hcd->is_irq_wake) {
		disable_irq_wake(tcm_hcd->irq);
		tcm_hcd->is_irq_wake = false;
		pr_notice("syna: disable_irq_wake");
	}
}
