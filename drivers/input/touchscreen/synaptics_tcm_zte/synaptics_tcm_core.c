...
static int syna_tcm_config_gpio(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (bdata->reset_gpio >= 0) {
		...
		pr_notice("%s: set reset gpio, state=%d\n", __func__, !bdata->reset_on_state);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
		...
		unsigned long action, void *data)
{
	...
	if (atomic_read(&tcm_hcd->firmware_flashing) && *transition == MSM_DRM_BLANK_POWERDOWN) {
		retval = wait_event_interruptible_timeout(tcm_hcd->reflash_wq,
				!atomic_read(&tcm_hcd->firmware_flashing),
				msecs_to_jiffies(RESPONSE_TIMEOUT_MS));
		if (retval == 0) {
...
static void syna_tcm_delay_probe(struct work_struct *work)
{
	...
	spin_lock_init(&tcm_hcd->edge_pre_lock);
...
#endif
	zte_edge_suppress_set(bdata->lcd_width, bdata->lcd_height,
						ZTE_SUPPRESS_LENGTH, DEFAULT_SUPPRESS_LEVEL, NULL);
