...
#define SYNA_MAX_POINTS_SUPPORT 10

/* zte: temporary add for under cell fingerprint */
#ifdef UNDER_FP
#define ZTE_CIRCLE_CENTER_X 540
#define ZTE_CIRCLE_CENTER_Y 2017
#define ZTE_CIRCLE_RADIUS 108
#endif

/* zte: add for uevent report */
...
#ifdef UNDER_FP
static char fp_finger[SYNA_MAX_POINTS_SUPPORT] = {0};
#endif
static unsigned int area_meet_state = 0;

static inline void report_gesture_uevent(struct syna_tcm_hcd *tcm_hcd, char *str)
{
	char *envp[2];

	envp[0] = str;
	envp[1] = NULL;
	kobject_uevent_env(&(tcm_hcd->pdev->dev.kobj), KOBJ_CHANGE, envp);
}

static inline void report_uevent(struct syna_tcm_hcd *tcm_hcd, char *str)
{
	char *envp[3];

	envp[0] = str;
	envp[1] = get_lcd_state();
	envp[2] = NULL;
	kobject_uevent_env(&(tcm_hcd->pdev->dev.kobj), KOBJ_CHANGE, envp);
}
...
static void touch_free_objects(void)
{
	...
	if (area_meet_state) {
		pr_notice("synapti: out zeon!\n");
		area_meet_state = 0;
		report_uevent(tcm_hcd, AREAMEET_UP);
	}
...
#ifdef UNDER_FP
/*zte: temporary add for under cell fingerprint*/
static inline int zte_in_zone(int x, int y)
{
	int ret = 0;
...
static void touch_report(void)
{
	...
	static char finger_down[SYNA_MAX_POINTS_SUPPORT] = {0};
	static struct syna_point_log point_log[SYNA_MAX_POINTS_SUPPORT];
	...
	if (tcm_hcd->is_single_tap
			&& touch_data->gesture_id == GESTURE_SINGLE_TAP && tcm_hcd->in_suspend) {
		pr_notice("synapti: single tap gesture\n");
		report_gesture_uevent(tcm_hcd, SINGLE_TAP_GESTURE);
		spin_lock(&tcm_hcd->fp_sintap_lock);
		tcm_hcd->is_off_single_tap = true;
		spin_unlock(&tcm_hcd->fp_sintap_lock);
		mod_delayed_work(tcm_hcd->single_tap_workqueue,
				&tcm_hcd->single_tap_work, msecs_to_jiffies(SINGLE_TAP_DELAY));
	}
	if (tcm_hcd->in_wakeup_gesture &&
		...
		report_gesture_uevent(tcm_hcd, DOUBLE_TAP_GESTURE);
		mod_delayed_work(tcm_hcd->single_tap_workqueue,
						&tcm_hcd->single_tap_work, 0);
	}
#endif

	if (touch_data->fingerprint_area_meet && !area_meet_state) {
		if (tcm_hcd->in_suspend && tcm_hcd->is_single_tap) {
			pr_notice("synapti: suspend in zeon\n");
			area_meet_state = 1;
			report_gesture_uevent(tcm_hcd, SINGLE_TAP_GESTURE);
			spin_lock(&tcm_hcd->fp_sintap_lock);
			tcm_hcd->is_off_single_tap = true;
			spin_unlock(&tcm_hcd->fp_sintap_lock);
			report_uevent(tcm_hcd, AOD_AREAMEET_DOWN);
		} else if (!tcm_hcd->in_suspend) {
			pr_notice("synapti: in zeon\n");
			area_meet_state = 1;
			report_uevent(tcm_hcd, AREAMEET_DOWN);
		}
	} else if (area_meet_state && !touch_data->fingerprint_area_meet) {
		pr_notice("synapti: out zeon!\n");
		area_meet_state = 0;
		report_uevent(tcm_hcd, AREAMEET_UP);
	}
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		case LIFT:
			...
			if (finger_down[idx]) {
				finger_down[idx] = 0;
				pr_notice("syna: touch up id: %d, coord[%d, %d]\n",
						idx, point_log[idx].x, point_log[idx].y);
			}
		...
		case GLOVED_FINGER:
...
#ifdef UNDER_FP
			retval = zte_in_zone(x, y);
			if (retval) {
				if (!fp_finger[idx] &&
						touch_data->fingerprint_area_meet && !area_meet_state) {
					fp_finger[idx] = 1;
					area_meet_state = 1;
					pr_notice("syna: in zeon\n");
					report_uevent(tcm_hcd, AREAMEET_DOWN);
				}
			} else if (fp_finger[idx] && !touch_data->fingerprint_area_meet) {
				fp_finger[idx] = 0;
				area_meet_state = 0;
				pr_notice("syna: out zeon!\n");
				report_uevent(tcm_hcd, AREAMEET_UP);
			...
			if (!finger_down[idx]) {
				finger_down[idx] = 1;
				pr_notice("syna: touch down id: %d, coord[%d, %d]\n", idx, x, y);
			}
			point_log[idx].x = x;
			point_log[idx].y = y;
			retval = zte_touch_report(x, y, idx, -1, -1, touch_hcd->input_dev);
			if (retval)
				object_data[idx].status = LIFT;
			else
				touch_count++;
...
#ifdef HBP_SUPPORT
static int syna_tcm_enable_frame_data(void)
{
	...
	touch_hcd->out.buf[0] = REPORT_FRAME_DATA;
...
#ifdef HBP_SUPPORT
	retval = syna_tcm_enable_frame_data();
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to enable frame report\n");
...
#ifdef WAKEUP_GESTURE
	status = tcm_hcd->is_single_tap | tcm_hcd->in_wakeup_gesture << 1;
	pr_notice("synapti: status = %d", status);
	mutex_lock(&tcm_hcd->extif_mutex);
	retval = tcm_hcd->set_dynamic_config(tcm_hcd,
			DC_ENABLE_FINGERPRINT_GESTURE,
			status);
	mutex_unlock(&tcm_hcd->extif_mutex);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to enable fp gesture mode\n");
		return retval;
	}
