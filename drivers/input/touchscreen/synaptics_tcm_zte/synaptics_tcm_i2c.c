...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	prop = of_find_property(np, "lcd_panel_width_height", NULL);
	if (prop && prop->length) {
		retval = of_property_read_u32_array(np, "lcd_panel_width_height",
				lcd_size, 2);
		if (retval < 0) {
			LOGE(dev,
					"Unable to read lcd_panel_width_height property\n");
			return retval;
		}
		bdata->lcd_width = lcd_size[0];
		bdata->lcd_height = lcd_size[1];
	} else {
		bdata->lcd_width = 1080;
		bdata->lcd_height = 2340;
	}
