...
void veb_qual_pin_init(struct spi_device *spi)
{
	...
	gpio_value = gpio_get_value_cansleep(reset_gpio);
	pr_info("reset gpio is %d\n", gpio_value);
	if (gpio_value != 1) {
		msleep(20);
		gpio_direction_output(reset_gpio, 1);
	}
...
#if defined(ZTE_FEATURE_PV_AR)
	int board_id;
	int ret = VEB_OK;

	board_id = request_board_id();
	if (board_id != 5) {
		VEB_ERR("in pv version but not das board_id:%d\n", board_id);
		ret = -ENODEV;
	} else {
		VEB_ERR("in pv version and das board_id:%d\n", board_id);
