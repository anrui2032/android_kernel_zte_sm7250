...
#define NVT_PINCTRL_SET_GPIO_MODE "nvt_set_spi_to_gpio_mode"
#endif
...
int nvt_platform_pinctrl_init(void)
{
	...
	ts->pinctrl_set_gpio_mode
	    = pinctrl_lookup_state(ts->ts_pinctrl, NVT_PINCTRL_SET_GPIO_MODE);
	if (IS_ERR_OR_NULL(ts->pinctrl_set_gpio_mode)) {
		ret = PTR_ERR(ts->pinctrl_set_gpio_mode);
		NVT_ERR("Can not lookup %s pinstate %d\n", NVT_PINCTRL_SET_GPIO_MODE, ret);
		goto err_pinctrl_lookup;
	}
...
int spi_gpio_mode_set(struct tpd_classdev_t *cdev, u8 mode)
{
	int ret = 0;

	if (mode == gpio_mode) {
		ret = pinctrl_select_state(ts->ts_pinctrl, ts->pinctrl_set_gpio_mode);
		if (ret < 0)
			NVT_ERR("failed to select pin to gpio mode");
	} else {
		ret = pinctrl_select_state(ts->ts_pinctrl, ts->pinctrl_state_init);
		if (ret < 0)
			NVT_ERR("failed to select pin to spi mode");
	}
	return ret;
}
#endif
...
#ifdef NVT_PINCTRL_EN
	tpd_fw_cdev.set_gpio_mode = spi_gpio_mode_set;
#endif
#if NVT_TOUCH_WDT_RECOVERY
